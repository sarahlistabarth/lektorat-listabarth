...
title: "Word-Makros"
toc: true
description-meta: Diese Makros helfen beim Lektorieren in Word.
---
...
Um die folgenden Makros zu installieren, gehen Sie folgendermaßen vor:

1.    Suchen Sie das Makro aus.
2.    Klappen Sie den VBA-Code aus (*VBA-Code zeigen*).
3.    Kopieren Sie den Code, indem Sie auf das Klemmbrett rechts klicken.
4.    Öffnen Sie den VBA-Code-Editor in Word: `Alt + F11` in Word drücken.
5.    Kopieren Sie den VBA-Code ganz unten in das Fenster `Normal - New Macros (Code)`.


6.    (*optional*) Weisen Sie dem Makro eine Tastenkombination zu, indem Sie auf `Datei` -- `Optionen` -- `Menüband anpassen` -- `Tastenkombinationen: Anpassen` gehen. 
Hier können Sie unter `Kategorien` -- `Makros` bei `Befehl` das Makro aussuchen.
Jetzt drücken Sie die Tastenkombination, die Sie haben mögen (z. B. `Strg + M`).
Als Letztes klicken Sie auf `Zuordnen`.
...
Bitte installieren Sie es daher gemeinsam mit diesem Makro.
:::
